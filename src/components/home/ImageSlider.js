...
const ImageSlider = ({ sliderImgs, layoutStyles }) => {
  ...
  useEffect(() => {
    ...
    if (!loading && activeTimers <= 1) {
      timeID = setInterval(() => {
        console.log("is paused?", paused)
        if (!paused) handleTimedSlide()
      }, 7000)
      setActiveTimers(prevState => prevState + 1)
      // setTime(interval)
    }
  }, [loading, scrollRef, paused])
  ...
  // if (isMobile) {
  //   return <MobileSlider layoutStyles={layoutStyles} sliderImgs={sliderImgs} />
  // }

  console.log(currentIndex, max, activeTimers, timeID, paused)
  return (
    <>
